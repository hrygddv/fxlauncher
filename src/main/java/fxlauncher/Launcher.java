...
public class Launcher extends Application {
                ...
                try {
                    if (Application.class.isAssignableFrom(appClass)) {
                        ...
                        throw new IllegalArgumentException(
                            String.format(
                                Constants.getString("Error.Application.Create.1"), appClass));
                    }
                } catch (Throwable t) {
    ...
    /**
     * Initialize the UI Provider by looking for an UIProvider inside the launcher or fallback to
     * the default UI.
     * <p>
        ...
        new Thread(() -> {
            ...
            try {
                ...
                log.log(Level.WARNING,
                    String.format(
                        Constants.getString("Error.Start.Phase"), superLauncher.getPhase()), ex
                );
                if (superLauncher.checkIgnoreUpdateErrorSetting()) {
                    superLauncher.reportError(
                        String.format(
                            Constants.getString("Error.Start.Phase"), superLauncher.getPhase()),
                        ex
                    );
                    System.exit(1);
                }
...
//		String fxSdkDir = superLauncher.getManifest().fxsdkDir;
        String command = String.format(
            "java --module-path \".\\javafx-sdk-19-x86\\lib\" --add-modules javafx.controls,javafx.fxml -cp \"%s\\*\" com.plgskyy.hospitalregistrationtool.MainApplication",
